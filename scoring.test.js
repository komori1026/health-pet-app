...
test("calculateTotalPoints は複数日を合算する", () => {
  const entries = {
    "2026-07-10": { tofu_first: 1, omega3: 6 },
...
test("getDailyAchievement はオメガ3を目標6錠未満はachieved:falseになる", () => {
  const omega3 = HABITS.find((h) => h.key === "omega3");
  const entries = { "2026-07-11": { omega3: 3 } };
  const result = getDailyAchievement(entries, "2026-07-11", omega3);
  assert.deepEqual(result, { actual: 3, target: 6, achieved: false });
});

test("getDailyAchievement はオメガ3を6錠飲めばachieved:trueになる", () => {
  const omega3 = HABITS.find((h) => h.key === "omega3");
  const entries = { "2026-07-11": { omega3: 6 } };
  const result = getDailyAchievement(entries, "2026-07-11", omega3);
  assert.deepEqual(result, { actual: 6, target: 6, achieved: true });
});
...
test("findCharacterImage は過去に画像が無ければ最も古い未来の画像にフォールバックする", () => {
  const entries = {
    "2026-07-14": { character_image: "history/2026-07-14.png" },
    "2026-07-16": { character_image: "history/2026-07-16.png" },
  };
  assert.equal(findCharacterImage(entries, "2026-07-13"), "history/2026-07-14.png");
});
...
test("getEarliestDateKey はentriesが空ならnullを返す", () => {
  assert.equal(getEarliestDateKey({}), null);
});

test("getEarliestDateKey はentriesの最も古い日付を返す", () => {
  const entries = {
    "2026-07-13": {},
    "2026-07-11": {},
    "2026-07-15": {},
  };
  assert.equal(getEarliestDateKey(entries), "2026-07-11");
});
