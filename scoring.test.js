...
test("getLatestComment はai_commentが無ければnullを返す", () => {
  const entries = { "2026-07-11": { aerobic: 10 } };
  assert.equal(getLatestComment(entries), null);
});

test("getLatestComment はai_commentが1件あればそれを返す", () => {
  const entries = {
    "2026-07-11": { aerobic: 10, ai_comment: "今日も一緒にがんばろう" },
  };
  assert.deepEqual(getLatestComment(entries), {
    dateKey: "2026-07-11",
    comment: "今日も一緒にがんばろう",
  });
});

test("getLatestComment は複数日から最新の日付のコメントを返す", () => {
  const entries = {
    "2026-07-10": { ai_comment: "古いコメント" },
    "2026-07-12": { ai_comment: "新しいコメント" },
    "2026-07-11": { ai_comment: "中間のコメント" },
  };
  assert.deepEqual(getLatestComment(entries), {
    dateKey: "2026-07-12",
    comment: "新しいコメント",
